...
const ChatDetail: NextPage<ChatDetailProps> = ({ chatRoomData }) => {
  ...
  const getLatestAppointment = useCallback(() => {
    ...
    // SellerChat에서 chatMeetup이 있는 메시지들을 찾기
    const appointmentMessages = data.sellerChat.filter(
      (message: ChatWithUser) =>
        message.chatMeetup && message.chatMeetup.appointmentTime
    );

    if (appointmentMessages.length === 0) return null;

    // 가장 최근 약속 찾기 (appointmentTime 기준)
    const latest = appointmentMessages[appointmentMessages.length - 1];
    return latest;
  }, [data?.sellerChat]);
