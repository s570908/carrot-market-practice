import { User } from "@components/User.server";
...
//         return (
//           <li key={i}>
//             <Suspense fallback={<p className="text-red-700">{`Todo id= ${i} is loading`}</p>}>
//               <Todo tid={i} />
//             </Suspense>
//           </li>
//         );
...
//   if (!todo) return null;
//   return <div>{`${todo?.title} : completed-${todo?.completed}`}</div>;
// }
...
export default function App() {
  return (
    <div className="App">
      <Suspense fallback={<p className="text-red-700">User is loading...</p>}>
        <User uid={1} />
      </Suspense>
    </div>
  );
