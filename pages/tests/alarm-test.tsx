...
const AlarmTest = () => {
  ...
  const [autoMonitorAlarmId, setAutoMonitorAlarmId] = useState<number | null>(
    null
  );
    ...
    try {
      ...
      if (testMode === "quick") {
        ...
        appointmentTime = new Date(
          `${realAppointmentDate}T${realAppointmentTime}`
        );
        alarmTime = realAlertOption;
        addTestResult(
          `실제 테스트 모드: ${appointmentTime.toLocaleString()} 약속, ${alarmTime} 알림`
        );
      }

      // 1. 테스트용 약속 메시지 먼저 생성
      addTestResult(
        `약속 생성 중... 약속시간: ${appointmentTime.toISOString()}`
      );
      ...
      addTestResult(
        `가상 약속 생성 완료: 약속 ID ${chatMeetupId}, 메시지 ID ${appointmentMessageId}`
      );

      // 2. 알림 메시지 생성
      const systemMessageResponse = await axios.post(
        "/api/chatroom/system-message",
        {
          chatRoomId: selectedRoom,
      ...
      addTestResult(
        `시스템 메시지 생성 완료: ID ${systemMessageResponse.data.systemMessage.id}`
      );
      return {
  ...
  const testAlarmSettings = async () => {
    try {
      ...
      if (testMode === "quick") {
        ...
        addTestResult(
          `빠른 알람 설정 중... 트리거 시간: ${triggerAt.toISOString()}`
        );
        ...
        addTestResult(
          `실제 알람 설정 중... 트리거 시간: ${triggerTimeFormatted}`
        );
        ...
        if (hoursUntilTrigger > 0) {
          addTestResult(
            `약 ${hoursUntilTrigger}시간 ${
              minutesUntilTrigger % 60
            }분 후에 알림이 울릴 예정입니다...`
          );
        } else {
          addTestResult(
            `약 ${minutesUntilTrigger}분 후에 알림이 울릴 예정입니다...`
          );
        }
      }
      ...
      const response = await axios.post(
        `/api/chatroom/${selectedRoom}/alarm-settings`,
        {
      ...
      if (!response.data.ok) {
        throw new Error(
          `알람 설정 실패: ${response.data.error || "알 수 없는 오류"}`
        );
      }
  ...
  const startCountdown = (seconds: number) => {
    ...
    const countdownInterval = setInterval(() => {
      ...
      if (remaining <= 0) {
        ...
        addTestResult(
          `⏰ 알림 시간이 되었습니다! 서버 알림이 발송되었는지 확인하세요.`
        );
      } else if (remaining <= 5) {
        // 5초 이하일 때만 카운트다운 표시
  ...
  const testApiPaths = async () => {
    ...
    try {
      ...
      const healthCheck = await axios
        ...
        .catch(() => ({ status: 404 }));
      addTestResult(
        `서버 상태 확인: ${healthCheck.status === 200 ? "정상" : "응답 없음"}`
      );

      // 채팅방 정보 확인
      const chatRoomInfo = await axios
        .get(`/api/chatroom/${selectedRoom}`)
        .catch((e) => ({ status: e.response?.status || 500 }));
      addTestResult(
        `채팅방 정보 확인: ${
          chatRoomInfo.status === 200 ? "정상" : "오류 " + chatRoomInfo.status
        }`
      );
    ...
    try {
      ...
      if (response.data.ok) {
        ...
        if (response.data.timeInfo) {
          const { formattedRemaining, direction, now, triggerAt } =
            response.data.timeInfo;
          const nowDate = new Date(now);
          const triggerDate = new Date(triggerAt);

          addTestResult(`🕐 현재 시간: ${nowDate.toLocaleTimeString()}`);
          addTestResult(
            `🔔 알림 예정 시간: ${triggerDate.toLocaleTimeString()}`
          );
        ...
        if (response.data.debug) {
          ...
          addTestResult(
            `📋 메타데이터: ${JSON.stringify(debugInfo).substring(0, 100)}...`
          );
        }
        ...
        if (status === "SCHEDULED" && response.data.timeInfo?.isPast) {
          addTestResult(
            `⚠️ 알림 시간이 지났지만 아직 SCHEDULED 상태입니다. 서버 스케줄러에 문제가 있을 수 있습니다.`
          );
        }
  ...
  const startMonitoring = (alarmId: number) => {
    ...
    addTestResult(
      `알림 ID ${alarmId}에 대한 자동 모니터링을 시작합니다 (30초 간격)`
    );
    ...
    monitorIntervalRef.current = setInterval(async () => {
      if (!loading) {
        ...
        if (
          status &&
          ["TRIGGERED", "DELIVERED", "CANCELED", "FAILED"].includes(status)
        ) {
          addTestResult(
            `알림 상태가 ${status}로 변경되어 모니터링을 중지합니다.`
          );
          stopMonitoring();
        }
      }
  ...
  const triggerAlarmManually = async (alarmId: number) => {
    try {
      ...
      if (response.data.ok) {
        ...
        addTestResult(
          `   - 트리거 시간: ${new Date(alarm.triggerTime).toLocaleString()}`
        );
        ...
        if (pushResult.sent) {
          ...
          toast.success(
            `알림이 수동으로 트리거되어 푸시 알림이 전송되었습니다!`
          );
        } else {
          addTestResult(
            `⚠️ 푸시 알림 전송 실패: ${pushResult.reason || "알 수 없는 이유"}`
          );
          toast.warning(
            `수동 트리거는 성공했지만 푸시 알림 전송에 실패했습니다: ${pushResult.reason}`
          );
        }
      ...
      if (axios.isAxiosError(error)) {
        ...
        addTestResult(
          `❌ HTTP ${statusCode}: ${errorData?.error || error.message}`
        );
      ...
      <div className="px-4 py-6">
            ...
            <div className="space-y-3 rounded-lg border border-gray-200 p-4">
              ...
              <div className="mt-2 rounded-md border border-yellow-200 bg-yellow-50 p-3 text-sm text-yellow-800">
                ...
                <ul className="ml-4 mt-1 list-disc">
                  <li>
                    정확한 테스트를 위해 약속 시간을 현재보다 2-3분 뒤로
                    설정하세요
                  </li>
                  <li>서버에서 알림이 발송되면 앱에서 확인할 수 있습니다</li>
                  <li>
                    알림 ID를 기록해두고 상태 확인 기능으로 서버 알림 상태를
                    확인하세요
                  </li>
                </ul>
              </div>
          ...
          <div className="mt-2">
            <button
              ...
              disabled={
                ...
                !selectedRoom ||
                (testMode === "real" &&
                  (!realAppointmentDate || !realAppointmentTime))
              }
              className={`w-full rounded-md py-4 font-medium text-white ${
                loading ||
                !selectedRoom ||
                (testMode === "real" &&
                  (!realAppointmentDate || !realAppointmentTime))
        ...
        {testResults.length > 0 && (
          <div className="mt-8">
            ...
            <div className="h-96 overflow-auto rounded-md border border-gray-200 bg-gray-50 p-4 text-sm text-gray-800">
        ...
        <div className="mt-4 rounded-lg border border-gray-200 p-4">
          ...
          <div className="mt-3 rounded-md border border-blue-200 bg-blue-50 p-3 text-sm text-blue-700">
            ...
            <ul className="ml-4 mt-1 list-disc">
              <li>
                <strong>SCHEDULED</strong>: 알림이 예약되었지만 아직 발송되지
                않음
              </li>
              <li>
                <strong>SENT</strong>: 서버에서 알림을 발송함 (수동/자동)
              </li>
              <li>
                <strong>DELIVERED</strong>: 클라이언트가 알림을 수신함
              </li>
        ...
        <div className="mt-4 rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-700">
          ...
          <ul className="ml-4 mt-1 list-disc">
            <li>
              알림 ID가 여전히 SCHEDULED 상태라면 서버 스케줄러에 문제가
              있습니다
            </li>
            <li>서버 재시작 후 스케줄러가 초기화되었을 가능성이 있습니다</li>
            <li>
              알림 처리를 위한 실시간 연결(WebSocket 등)이 구현되어 있지 않을 수
              있습니다
            </li>
